...
def pagerank(graph, damping_factor=0.85, max_iterations=100, \
    ...
    w_file = open('./test/diff-ttr-100.dat','w')
...
def init_metadata(ratio_file, as_file, sim_file, jump_file):
	"""
	...
	# laod active similarity
	global AS
	AS = {}
	file_as = open(as_file, 'r')
	for line in file_as:
		if line.strip() =='':
			continue
		items = line.split()
		AS[items[0] + '_' + items[1]] = float(items[2])
	file_as.close()
...
def afterRank(follower_id, friend_id, file_name):
	...
	result = np.argsort(-dist,axis=0) # sort align x with descending orde.r Returns the indices that would sort an array 
...
def afterRank2(file_name):
	...
	for line in r_file:
		items = line.split()
		file_name = items[0] + '_' + items[1] + '.rank'
		afterRank(items[1], items[0], file_name)
	r_file.close()
	
if __name__=='__main__':
	#afterRank2('./test/Lfl.txt')
	loop = ['Lth-ttr.txt'] # ,'Lfl.txt','Lth.txt','Ltl.txt','Ldh.txt','Ldl.txt'
	for v in loop:
		print v
		r_file = open('./test/' + v, 'r')
		for line in r_file:
			gr = init_graph('nodes.dat','edges.dat', line)
			init_metadata('i-j-ratio.dat', 'i-j-cosine.dat', 'sim-100.dat', 'jump-100.dat')
			rank = pagerank(gr)
			items = line.split()
			file_name = items[0] + '_' + items[1] + '.rank'

...
def count_reply_delay():
	conn=MySQLdb.connect(host='localhost',user='root',passwd='root',db='twitter',port=3306)
	cur=conn.cursor()
	cur.execute('select user_id,friend_id,tweet_id,reply_id,unix_timestamp(tweet_time) as tweet_time, unix_timestamp(reply_time) as reply_time from count_reply_delay order by user_id')
	count=0
	sorted_time_list = []
	user_id='test.txt'
	w_file = open('./count_reply_lookback.txt','w')
...
def store_into_db():
	r_file = open('./count_reply_lookback.txt', 'r')
	conn=MySQLdb.connect(host='localhost',user='root',passwd='root',db='twitter',port=3306)
	cur=conn.cursor()
	count=0
	update_list = []
	for line in r_file:
		...
		count+=1
		if count%1000 == 0:
			print 'update %s reply lookback...'%count
			cur.executemany('UPDATE count_reply_delay SET tweet_count=%s WHERE user_id=%s AND friend_id=%s AND tweet_id=%s AND reply_id=%s',update_list)
			update_list = []
			print 'done!'
